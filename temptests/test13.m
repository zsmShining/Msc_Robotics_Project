...
Kp = 1.0;
track = [];
follower1 = agent(2,"follower",[-4,-1,0,0,0],[-10,5]);
follower2 = agent(2,"follower",[4,-1,pi/3,0,0],[-10,5]);
follower1 = follower2;
dt = 0.1;
...
KMatrix = [1,0.4,0.1;1,0.5,0.1;0.9,0.5,0.01];
% KMatrix = [4,1,0.5;5.0,0.5,0.1;0.9,0.5,0.01];
Boundary = [0,2.5;-1,1;-1,1];
KMatrix = [4,1,0.1;2,1.5,0.1;1,0.5,0.1];Boundary = [0,5;-10,10;-10,10];
Lamda = [1,2];
...
trackJ = [leader.position,follower1.position];
for t = 1:simStep
    Tool_lib.plotScene(fleet.agent,colorList);
    Tool_lib.plotScene({follower1,follower2},colorList);
...
%     plot(trackJ(:,5),trackJ(:,6),"-");
    ...
    Error = follower1.calPoseErrorinLocalFrame(virAgt);
%    track = [track;Error'];
    ...
    des_vel = piddes_vel;
    des_omega = res(2);
    follower1 = follower1.set("linearSpeed",piddes_vel);
    follower1 = follower1.set("angularSpeed",des_omega);
    des_ang=Object_lib.convertBoundAng(des_omega*dt + follower1.orientation);
    des_vel = des_vel * [cos(des_ang),sin(des_ang)];
    ndes_vel=follower1.calAvoidCollision({obs2},des_vel);
    ...
    track = [track;vel];
    follower1 = follower1.set("linearSpeed",vel);
    old = follower1.orientation;
    ...
    disp(omega)
    follower1 = follower1.set("angularSpeed",omega);
    omega_track = [omega_track,omega];
 
    follower1 = follower1.nextStep(dt);
    fleet = formation_lib.update(fleet,leader);
    trackJ = [trackJ;leader.position,follower1.position];
    if t == simStep
        break;
    else
        clf();
