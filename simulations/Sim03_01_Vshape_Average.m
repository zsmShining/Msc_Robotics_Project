...
% fo2 = agent(3,"follower",[-1,-9,pi/2],[0,0]);
fo1 = agent(2,"follower",[3.53,-3.53,pi/2],[0,0]);
fo2 = agent(3,"follower",[-3.53,-3.53,pi/2],[0,0]);
f1ErrorTrack = [];
f2ErrorTrack = [];
follower_track = [];
fo1StateTrack = [];
...
%% setup the obstacle
% static
obs1 = obstacle(4,[-5,5,0,0,0],1);
obs2 = obstacle(5,[0,10,0,0,0],1);
obs3 = obstacle(6,[-5,10,0,0,0],1);
obs4 = obstacle(6,[-10,5,0,0,0],1);

% obs5 = obstacle(7,[5,0,pi,1,0],1);
obs6 = obstacle(8,[-20,20,-pi/8,1,0],1);
obsList = {obs1,obs2,obs3,obs4,obs6};
obs5Track = [];
obs6Track = [];
%% setup simulation
dt = 0.1;
...
for t = 0:simStep
    ...
    ACvector = leader.calAvoidCollision(obsList,MGvector);
    finalVector = (MGvector)/4+(ACvector)*3/4;
    ...
    f1ACvector = fo1.calAvoidCollision({obs1,obs2,obs3,obs4},FK1vector);
    finalVector = (FK1vector)/4 + (f1ACvector)*3/4;
    [f1AC_vel_c,f1AC_omega_c] = convertGuideVector(fo1,finalVector,dt,BoundFollower);
    if abs(f1AC_omega_c) > 10
        f1AC_omega_c = sign(f1AC_omega_c) * 10;
    end
    ...
    f2ACvector = fo2.calAvoidCollision(obsList,FK2vector);
    finalVector = (FK2vector)/4+(f2ACvector)*3/4;
    [f2AC_vel_c,f2AC_omega_c] = convertGuideVector(fo2,finalVector,dt,BoundFollower);
    % Follower2 take in command to move
    if abs(f2AC_omega_c) > 10
        f2AC_omega_c = sign(f2AC_omega_c) * 10;
    end
...
%    obsList{end-1} = obs5;
    obsList{end} = obs6;
%    obs5Track = [obs5Track;obs5.position];
...
%plot(obs5Track(:,1),obs5Track(:,2),"c");
