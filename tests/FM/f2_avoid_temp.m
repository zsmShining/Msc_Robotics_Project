...
obs3 = obstacle(6,[-7,12,0,0,0],0.5);
%% setup simulation
dt = 0.1;
simStep = 400;
...
%KMatrix = [4,0.1,0;1.5,0.5,0;2,1,0.1];
KMatrix=[4,1,0.1;2,1.5,0.1;1,0.5,0.1];
Boundary1 = [0,5;-1.5,1.5;-1,1];

Lamda1 = [2,2];
ctrF1 = formation_controller3(KMatrix,Boundary1,Lamda1,dt,false);

%% setup Formation Controller2
%KMatrix2 = [4,1,0.01;2,0.5,0.01;2,0.5,0.01];
KMatrix2 = [4,0.4,0.1;2.0,0.5,0.1;2.0,1,0.2];
...
for t = 0:simStep
    ...
    Tool_lib.plotScene({obs1,obs2,obs3},['m','m','m']);
    drawnow;
    if t~= simStep
        clf();
    else
    ...
    ACvector = leader.calAvoidCollision({obs1,obs2,obs3},MGvector);
    [AC_vel_c,AC_omega_c] = convertGuideVector(leader,ACvector,dt,BoundLeader);
    ...
    f1ACvector = fo1.calAvoidCollision({obs1,obs2,obs3},FK1vector);
