...
%% setup the obstacle
% static
obs1 = obstacle(4,[-5,5,0,0,0],0.5);
obs2 = obstacle(5,[0,10,0,0,0],0.5);
...
for t = 0:simStep
    ...
    Tool_lib.plotScene({obs1,obs2,obs3,obs4},['m','m','m','m']);
    drawnow;
    ...
    ACvector = leader.calAvoidCollision({obs1,obs2,obs3,obs4},MGvector);
    ...
    f2ACvector = fo2.calAvoidCollision({obs1,obs2,obs3,obs4},FK2vector);
...
dis = [];
for i = 1:length(f1ErrorTrack)
    disError = norm(f1ErrorTrack(i,1),f1ErrorTrack(i,2));
    dis = [dis;disError];
end
...
dis = [];
for i = 1:length(f1ErrorTrack)
    disError = norm(f2ErrorTrack(i,1),f2ErrorTrack(i,2));
    dis = [dis;disError];
end
plot(1:length(dis),dis);
